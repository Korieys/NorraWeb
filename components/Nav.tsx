...
export function Nav() {
  ...
  // Portalled so they escape any ancestor with a CSS transform (e.g. the
  // page-in animation wrapper in template.tsx). A transform on an ancestor
  // makes it the containing block for position:fixed children, breaking
  // viewport-relative positioning when the page is scrolled.
  const mobileOverlays = mounted
    ? createPortal(
        <>
          <div
            id="mobile-nav"
          ...
          {open && (
            <div
              aria-hidden
              className="fixed inset-x-0 bottom-0 top-[72px] z-30 bg-ink/20 md:hidden"
              onClick={close}
            />
          )}
        </>,
